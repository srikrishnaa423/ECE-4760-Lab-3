...
static PT_THREAD (protothread_main(struct pt *pt))
{
    ...
    ADC_scale = float2fix16(210.0/990.0); //Vref/(full scale)
    ...
    while(1) {       
        ...
        output_TFT(275,20,actual_frame_rate);
        output_TFT(275,40,num_balls);
        output_TFT(275,60,score);
        
        execution_time = PT_GET_TIME() - begin_time;
        output_TFT(275, 80, execution_time);
        
        if(execution_time>56)
            actual_frame_rate = 1000/execution_time;
        else
            actual_frame_rate = framerate;
                
        //inject_2balls();
        PT_YIELD_TIME_msec(56 - (execution_time)) ;
